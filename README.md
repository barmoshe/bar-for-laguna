...
A one-page application site, Bar Moshe, presented in Laguna Health's own brand, sent
...
All brand values live in `src/marketing/laguna/laguna.css` (`.laguna-root` tokens).
...
```bash
npx vercel --prod          # → https://bar-for-laguna.vercel.app
```
