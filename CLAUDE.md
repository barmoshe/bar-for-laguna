# CLAUDE.md: bar-for-laguna (application site)
...
**Replicate the company, do not recolour the laguna.** Everything a visitor
...
live off their site. The laguna only gives you the plumbing. See `BRAND-READ.md`
...
- Next.js 16 (App Router, Turbopack) + React 19 + TypeScript. Plain CSS, **no
  Tailwind**. Deployed on Vercel at `https://bar-for-laguna.vercel.app`.
- Brand tokens live at the top of `src/marketing/laguna/laguna.css`, scoped to
  `.laguna-root`. Re-brand from there.
- `src/marketing/laguna/marketing-base.css` is the brand-agnostic reset + a11y
...
- The GH-Pages export fallback (ADR 0169) is wired in `next.config.ts`; the CV
  link is base-path-safe in `LagunaApp.tsx`.
