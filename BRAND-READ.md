# BRAND-READ: replicate the company, don't recolour the laguna

The laguna gives you the plumbing (config, a11y, reveal, CTAs, section spine).
...
motion feel, not the laguna with new colours. Work top to bottom.
...
- **Colours**: `getComputedStyle` on `body`, headings, buttons, links for exact
  hex. Fill `--laguna-bg / --laguna-surface / --laguna-border / --laguna-ink / --laguna-ink-soft /
  --laguna-accent / --laguna-accent-ink` in `src/marketing/laguna/laguna.css`.
- **Type**: the real display + body faces. Their brand face is often licensed;
  ...
  Lato). Expose it as `--laguna-font`.
- **Shape**: corner radius (`--laguna-radius`; `0` for square brands like WeDev),
...
spine in `LagunaApp.tsx` to mirror how *they* structure a page.
